...
public class AdminFormDialog extends JDialog {
    ...
    private void doSave() {
        try {
            ...
            if (full.isEmpty() || mail.isEmpty()) throw new IllegalArgumentException("Điền đầy đủ các trường (*)!");
            if (original == null) {
                String user = txtUsername.getText().trim();
                String pass = new String(txtPass.getPassword());
                String conf = new String(txtConfirm.getPassword());
                if (user.isEmpty()) throw new IllegalArgumentException("Tên đăng nhập không được trống!");
                if (pass.length() < 6) throw new IllegalArgumentException("Mật khẩu ít nhất 6 ký tự!");
                if (!pass.equals(conf)) throw new IllegalArgumentException("Mật khẩu không khớp!");
