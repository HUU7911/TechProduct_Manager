...
public class ProductPanel extends JPanel {
    ...
    private void search() {
        load(ps.searchProducts(txtSearch.getText().trim()));
    }
    ...
    private void showAdd() {
        ...
        if (d.getResult() != null)
            try {
                ps.addProduct(d.getResult()); load(ps.getAllProducts()); ok("Thêm thành công!");
            } catch (Exception ex) {
    ...
    private void showEdit(int row) {
        ...
        if (d.getResult() != null)
            try {
                ps.updateProduct(d.getResult()); load(ps.getAllProducts()); ok("Cập nhật thành công!");
            } catch (Exception ex) {
                GlobalExceptionHandler.handle(this, ex);
            }
    }
    ...
    private void doDel(int row) {
        ...
        if (JOptionPane.showConfirmDialog(this, "Xóa sản phẩm: " + p.getName() + "?",
            ...
            try {
                ps.deleteProduct(p.getId()); load(ps.getAllProducts()); ok("Đã xóa!");
            } catch (Exception ex) {
                GlobalExceptionHandler.handle(this, ex);
            }
    }
    ...
    private void ok(String m) {
        ...
        new Timer(3000, e -> {
            lblStatus.setForeground(UITheme.TEXT_DIM); lblStatus.setText("Hiển thị " + current.size() + " sản phẩm");
        }).start();
    }
}