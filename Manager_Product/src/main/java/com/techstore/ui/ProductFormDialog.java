    ...
    private void doSave() {
        try {
            ...
            if (name.isEmpty() || brand.isEmpty() || ps.isEmpty() || qs.isEmpty())
                throw new AppException("Vui lòng điền đầy đủ các trường bắt buộc (*)!", ErrorCode.VALIDATION_ERROR, null);
            double price = Double.parseDouble(ps);
            int qty = Integer.parseInt(qs);
            if (price < 0) throw new AppException("Giá không được âm!", ErrorCode.VALIDATION_ERROR, null);
            if (qty < 0)   throw new AppException("Số lượng không được âm!",  ErrorCode.VALIDATION_ERROR, null);
            if (original == null) {
