...
public class RegisterDialog extends JDialog {
    ...
    private void doRegister() {
        try {
            ...
            String pass = getString(user, full, mail, txtPassword, txtConfirm);
    ...
    private static String getString(String user, String full, String mail, JPasswordField txtPassword, JPasswordField txtConfirm) {
        String pass = new String(txtPassword.getPassword());
        String confirm = new String(txtConfirm.getPassword());

        if (user.isEmpty() || full.isEmpty() || mail.isEmpty()) {
            throw new AppException("Vui lòng điền đầy đủ các trường *", ErrorCode.VALIDATION_ERROR, null);
        }
        if (pass.length() < 6) {
            throw new AppException("Mật khẩu phải ít nhất 6 ký tự!", ErrorCode.VALIDATION_ERROR, null);
        }
        if (!pass.equals(confirm)){
            throw new AppException("Mật khẩu xác nhận không khớp!", ErrorCode.VALIDATION_ERROR, null);
        }
        return pass;
    }
}